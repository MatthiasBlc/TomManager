...
- [ ] Migration Prisma additive : `Meal.vegeCount` / `Meal.carneCount`
      ...
      sur la base docker.
- [ ] Verifier si `discord-bot/prisma/schema.prisma` duplique le model
      `Meal` ; repercuter si oui.
- [ ] `updateMealSchema` (`backend/src/schemas/kitchen.ts`) : ajouter
      `vegeCount`/`carneCount` (`z.number().int().min(0).optional()`).
- [ ] `updateMeal` (`backend/src/services/meal.ts`) : ajouter les deux
      champs a `touchesManagerOnly` (memes droits que `maxAssistants`) et a
      la data d'update Prisma.
- [ ] `getKitchenView`/`computeRosterLists` (`backend/src/services/kitchen.ts`) :
      exposer `eventParticipantsCount` (= `participations.length`, deja
      recupere) au niveau racine de la reponse GET /kitchen.
- [ ] Projection des repas (`services/meal.ts`, `services/kitchen.ts`) :
...
- [ ] Tests integration : validation min(0), 403 si un chef tente de
      ...
      equipier.
- [ ] Mettre a jour `.claude/context/DB_MODELS.md` et `API_MAP.md`.

## Lot B - UI Gestion (auto-equilibrage + warning)

- [ ] `MealFichesList.tsx` : nouvelle ligne "Repas" entre "Places" et
      "Equipiers" - deux `NumberStepper` (vege/carne), PATCH immediat par
      champ (meme pattern que la capacite equipiers existante).
- [ ] Logique d'auto-equilibrage front : editer un champ met l'autre a
      `eventParticipantsCount - valeur`, clamp `[0, eventParticipantsCount]`.
- [ ] Bandeau warning (badge + texte) si
      `vegeCount + carneCount !== eventParticipantsCount`, y compris a
      l'etat initial 0/0.
- [ ] S'assurer que les repas orphelins (sans chef) affichent aussi cette
      ligne, editable par le responsable.
- [ ] Tests frontend (testing-library) : auto-equilibrage bidirectionnel,
      warning affiche/masque selon la somme, clamp aux bornes.

## Lot C - Notifications

- [ ] `updateMeal` (`services/meal.ts`) : comparer `vegeCount`/`carneCount`
      ...
      CLAUDE.md).
- [ ] Verifier qu'aucune notif n'est envoyee si le repas est orphelin ou si
      les valeurs n'ont pas change (ex: PATCH avec les memes valeurs).
- [ ] Reutiliser `kitchen:meal-changed` (deja emis) pour le temps reel, pas
      de nouvel event socket.
- [ ] Tests integration : notif creee (bon destinataire/contenu) sur
...
- [ ] `MealFicheEditor.tsx` : bloc lecture seule (2 stats vege/carne) pres
      du badge de capacite equipiers existant.
- [ ] Note informative discrete si mismatch (non bloquante, non
      actionnable par le chef).
- [ ] Tests frontend : rendu correct des valeurs, note affichee seulement
      en cas de mismatch.

## Lot E - Dashboard admin simple

- [ ] `KitchenDashboard.tsx` : ajouter `vegeCount`/`carneCount` a
      l'interface `DashboardMeal` et deux badges lecture seule par carte
      repas (meme niveau d'info que Chef/Places).
- [ ] Tests frontend : badges presents, aucune action/edition possible.

---

## E2E (a la fin, Lot B+C+D+E fusionnes dans le parcours existant)

- [ ] Etendre `e2e/cuisine.spec.ts` (ou nouveau spec dedie) : responsable
...
- [ ] `.claude/context/PROGRESS.md` : pointer vers cette roadmap tant que
      des lots restent ouverts, puis retirer une fois merge.
- [ ] `.claude/context/TESTS.md` si de nouveaux fichiers de test sont crees.
