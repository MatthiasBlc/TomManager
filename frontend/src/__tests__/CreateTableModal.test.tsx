...
describe("CreateTableModal", () => {
  ...
  it("renders nothing when closed", () => {
    render(
      <CreateTableModal open={false} onClose={vi.fn()} onCreated={vi.fn()} eventId="ev1" />
    );
  ...
  it("renders the form when open", () => {
    render(
      <CreateTableModal open={true} onClose={vi.fn()} onCreated={vi.fn()} eventId="ev1" />
    );
  ...
  it("submits the table and calls onCreated/onClose on success", async () => {
    ...
    const onClose = vi.fn();
    render(
      <CreateTableModal open={true} onClose={onClose} onCreated={onCreated} eventId="ev1" />
    );
  ...
  it("calls onClose when Annuler is clicked", () => {
    const onClose = vi.fn();
    render(
      <CreateTableModal open={true} onClose={onClose} onCreated={vi.fn()} eventId="ev1" />
    );
