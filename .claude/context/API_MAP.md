...
| Method | Path                                | Auth                                   | Description                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------ | ----------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PATCH  | `/meals/:mealId`                    | requireAuth + requireMealChefOrManager | Edite un repas ; le chef proprietaire n'edite que `name`/`recipe`/ingredients/ustensiles. `service`, `startDateTime`, `endDateTime`, `maxAssistants`, `vegeCount`/`carneCount` (KitchenDietSplit), `chefUserId` (reassignation d'un orphelin, 400 `MEAL_NOT_ORPHAN`) reserves au manager (403 `FORBIDDEN`). Notif `KITCHEN_DIET_SPLIT_UPDATED` au chef si vegeCount/carneCount changent et repas non orphelin |
...
unit, note?}]`, `utensils: [{name}]`), remplacement complet a chaque appel (delete+recreate). Les
ingredients font un find-or-create sur `Product` (nom normalise lowercase, cf `/api/kitchen/products`).
**L'ordre du tableau `ingredients` fait foi** : il est persiste dans `MealIngredient.position`
(index du tableau) et toutes les lectures trient dessus (GET /kitchen, fiche, vues Courses,
export Excel). Le chef reorganise ses lignes au glisser-deposer (desktop) ou aux fleches
(partout, y compris clavier).

`recipe` : bloc-notes texte libre du chef (recette collee, deroule, remarques), max 10000
caracteres, `null`able. Editable par le chef proprietaire comme par le responsable (pas un
champ manager-only) ; expose en lecture aux seuls `isFullReader` (chef + responsable), comme
les ingredients/ustensiles. Une chaine vide ou blanche est stockee `null`.
...
- `byMeal` : groupes chronologiques, ingredients dans l'ordre compose par le chef
  (`MealIngredient.position`, comme `GET /kitchen`) ; **un repas sans ingredient est
  conserve**.
