...
- `IngredientListInput.test.tsx` - Quantite virgule ET point acceptees (point 8), pas de commit sur saisie non-parsable ; commentaire par ingredient (masque tant qu'il n'est pas demande, remonte la saisie, reaffiche un commentaire existant, RETIRE un commentaire et replie la zone) ; reorganisation des lignes (fleches haut/bas, bornes desactivees, commentaire qui suit sa ligne apres deplacement, reorganisation au drop) ; debit de l'autocompletion produit (pas de recherche sous 2 caracteres, une seule requete pour un nom tape en continu)
